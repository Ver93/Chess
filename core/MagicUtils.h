...
namespace MagicUtils {
    ...
    inline uint64_t generateJumpingMoves(int square, int pieceType, bool isWhite){
    ...
    bool testMagic(std::vector<uint64_t>& permutations, std::vector<uint64_t>& used, uint64_t magic, int relevantBits, int pt, int square, int arraySize) {
        ...
        inline std::vector<uint64_t> calcTotalPermutations(uint64_t mask) {
    ...
    inline uint64_t randomMagic(){
        return rand64() & rand64() & rand64();
    }

    inline bool filterHeuristics(uint64_t magic, uint64_t mask, int shift){
        return (__builtin_popcountll((magic * mask) & 0xFF00000000000000ULL) < shift);
    }
    ...
    inline uint64_t findMagic(int square, int relevantBits, int pt, uint64_t& mask, std::vector<uint64_t> permutations) {
        ...
        inline void saveMagicData(std::unique_ptr<AttackData>& dataPtr, const std::string& saveString) {
        if (!dataPtr) {
    ...
    inline std::unique_ptr<AttackData> loadMagicData(const std::string& loadString) {
